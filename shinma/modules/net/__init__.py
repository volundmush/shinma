...
class Connection:
    ...
    def oob(self):
        return self.msdp or self.gmcp or self.protocol.upper() == "WEBSOCKET"
    ...
    async def msg_client_capabilities(self, msg):
        ...
        self.engine.dispatch_module_event("net_client_reconfigured", connection=self)
    ...
    async def msg_client_disconnected(self, msg):
        ...
        """
        self.engine.dispatch_module_event("net_client_disconnected", reason=msg["reason"], connection=self)
        self.stop()
    ...
    async def msg_client_lines(self, msg):
        for line in msg["lines"]:
            self.engine.dispatch_module_event("net_client_command", text=line, connection=self)

    def start(self):
        self.engine.dispatch_module_event("net_client_connected", connection=self)
    ...
    def stop(self):
        if self.task:
            self.task.cancel()
        self.module.connections.pop(self.name)
...
class Module(ShinmaModule):
    ...
    def __init__(self, engine, *args, **kwargs):
        ...
        self.incoming_queue = Queue()
        self.outgoing_queue = Queue()
        engine.subscribe_event("net_connection_kick", self.net_connection_kick)
        engine.subscribe_event("net_connection_data", self.net_connection_data)
        self.switchboard = {
    ...
    def net_connection_kick(self, event, *args, **kwargs):
    ...
    def net_connection_data(self, event, *args, **kwargs):
        if not (conn := self.connections.get(kwargs["id"])):
            return
        conn.msg(kwargs["data"])

    def init_settings(self, settings):
        settings.PORTAL = "http://127.0.0.1:7998"
    ...
    async def create_client(self, data):
        for result in self.engine.dispatch_module_event("net_client_allow_connect", data):
        ...
        while running:
            try:
                async with ClientSession(loop=asyncio.get_event_loop()) as session:
                    async with session.ws_connect(self.engine.settings.PORTAL) as ws:
                        self.ws_link = ws
                        async for msg in ws:
